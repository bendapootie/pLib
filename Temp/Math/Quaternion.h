...
public:
	...
	// quaternion conjugate is (-x,-y,-z, w)
	// Returns the conjugate of this quaternion
	Quaternion GetConjugate() const;
	// Makes the quaternion the conjugate of itsself
	void MakeConjugate();

	Quaternion operator * (const Quaternion& other) const;

	// Sets the quaternion equivalent to the euler angles passed in
	void FrowEulerAngles(float theta_z, float theta_y, float theta_x);

	// Somewhat hacky functions to get a 'float' pointer to the start of the Quaternion
	const float* Ptr() const { return &x; }
	float* Ptr() { return &x; }

	// Returns a copy of the Quaternion in Vector4 format
	Vector4 ToVector4() const;
	// Returns a reference to this Quaternion in Vector4 format
	Vector4& AsVector4();
	const Vector4& AsVector4() const;
	// Returns a copy of the (x,y,z) components in Vector3 format
	Vector3 ToVector3() const;
	// Returns reference to the (x,y,z) components of this Quaternion as a Vector3
	Vector3& AsVector3();
	const Vector3& AsVector3() const;

	Vector3 RotateVector(const Vector3& input) const;

	// Returns a 4x4 Matrix that represents the same rotation as the Quaternion
